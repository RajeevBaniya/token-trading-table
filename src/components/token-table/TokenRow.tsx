...
function getMockTimestamp(): string {
  const timestamps = ['47s', '1m', '3m', '19h', '59m', '1h', '2h', '3h'];
  return timestamps[Math.floor(Math.random() * timestamps.length)];
}

function getMockEngagement(): { person: number; globe: number; search: number; doc: number; crown: number } {
  return {
    person: Math.floor(Math.random() * 10) + 1,
    globe: Math.floor(Math.random() * 5) + 1,
    search: Math.floor(Math.random() * 20) + 1,
    doc: Math.floor(Math.random() * 500) + 50,
    crown: Math.floor(Math.random() * 200) + 10,
  };
}

function getMockTxCount(): number {
  return Math.floor(Math.random() * 8000) + 3;
}

function TokenRow({ token }: TokenRowProps) {
  const timestamp = getMockTimestamp();
  const engagement = getMockEngagement();
  const txCount = getMockTxCount();
  const percentage = Math.floor(Math.random() * 100);

  return (
    <div className="border-b border-gray-800 p-3 sm:p-4 hover:bg-gray-800/30 transition-colors cursor-pointer">
      <div className="flex items-start justify-between gap-2 sm:gap-3 mb-2 sm:mb-3">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[10px] sm:text-xs text-gray-500 mb-1 sm:mb-2">{timestamp}</div>
            ...
            <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3 flex-wrap">
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-gray-400">{engagement.person}</span>
              </div>
              <div className="flex items-center gap-1">
                <svg className="w-3 h-3 text-gray-500" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM4.332 8.027a6.012 6.012 0 011.912-2.706C6.512 5.73 6.974 6 7.5 6A1.5 1.5 0 019 7.5V8a2 2 0 004 0 2 2 0 011.523-1.943A5.977 5.977 0 0116 10c0 .34-.028.675-.083 1H15a2 2 0 00-2 2v2.197A5.973 5.973 0 0110 16v-2a2 2 0 00-2-2 2 2 0 01-2-2 2 2 0 00-1.668-1.973z" clipRule="evenodd" />
                </svg>
                <span className="text-xs text-gray-400">{engagement.globe}</span>
              </div>
              <div className="flex items-center gap-1">
                <svg className="w-3 h-3 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
                <span className="text-xs text-gray-400">{engagement.search}</span>
              </div>
              <div className="flex items-center gap-1">
                <svg className="w-3 h-3 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <span className="text-xs text-gray-400">{engagement.doc}</span>
              </div>
              <div className="flex items-center gap-1">
                <svg className="w-3 h-3 text-gray-500" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
                <span className="text-xs text-gray-400">{engagement.crown}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 mb-2">
              <span
                className={`text-sm font-semibold ${
                  percentage >= 50 ? 'text-green-400' : percentage >= 20 ? 'text-gray-400' : 'text-red-400'
                }`}
              >
                {percentage}%
              </span>
      ...
      <div className="flex items-center justify-end">
        <button
          className="px-2 sm:px-3 py-1 bg-blue-600 hover:bg-blue-700 rounded text-[10px] sm:text-xs text-white font-medium transition-colors"
          onClick={(e) => e.stopPropagation()}
        >
          4 0 {token.chain}
        </button>
